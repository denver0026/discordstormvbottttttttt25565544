...
exports.run = (client, message, args) => {
  ...
  let modlog = guild.channels.find('name', 'st-log');
  let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'sus');
  if (!modlog) return message.reply('`st-log` kanalını bulamıyorum.').catch(console.error);
  if (!muteRole) return message.reply('`Sus` adlı bir rol bulamıyorum.').catch(console.error);
